...
class CustomerServiceTest {
    ...
    @Test
    fun `givenValidUserId_whenCallFindAllByUserId_shouldSuccess`() {
        val validUUID = UUID.randomUUID()
        val validUserId = 1L
        val customer = Customer(
                id = validUUID,
        ...
        val pageResponse = PageImpl<Customer>(customers)

        every { customerRepository.findAllByUserId(any(), any()) } returns pageResponse

        customerService.findAll(validUserId, 0, 20).let { customerPageResponse ->
            assertNotNull(customerPageResponse)
            assertFalse(customerPageResponse.isEmpty)
        }
    }
}
